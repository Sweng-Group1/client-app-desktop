...
public class UserService {
	...
	private void loadURLs(String urlsPath) throws RuntimeException {
		Properties urlProps = new Properties();
		try {
			urlProps.load(new FileInputStream(urlsPath));
		} catch (FileNotFoundException e) {
			e.printStackTrace();
			throw new RuntimeException("Couldn't find URL file.");
		} catch (IOException e) {
			e.printStackTrace();
			throw new RuntimeException("Couldn't load URL file.");
		}
	...
	/**
	 * Logs the user in to the application. If successful, will save the access and
	 * refresh tokens for the user locally (per-user files).
	 * 
	 * @param user     The user you are attempting to login as.
	 * @param password User's password.
	 * @return Returns the status code (standard HTTP codes, e.g. 200 success, 403
	 *         incorrect credentials), or 0 if an error occurs.
	 * @throws AuthenticationException Occurs when the user logs in with incorrect
	 *                                 credentials.
	 * @throws IOException             Can occur due to file system errors saving
	 *                                 the tokens, or network errors with the
	 *                                 server.
	 */
	public int login(User user, String password) throws AuthenticationException, IOException {
		...
		Request request = new Request.Builder().url(loginURL)
				...
				.post(RequestBody.create(requestBody, MediaType.parse("application/x-www-form-urlencoded"))).build();
	 ...
	 * @param password The user's password. Encryption occurs on the server. 
	 * @param firstName The user's first name.
	 * @param lastName The user's last name. 
	 * @param email the user's email address. 
	 * @return 200 if successful. 0 if a field was blank - fields cannot be blank. Will throw exceptions otherwise. 
	 * @throws IOException
	 */
	public int createUser(String username, String password, String firstName, String lastName, String email)
		...
		if (username.isBlank() == true || password.isBlank() == true || firstName.isBlank() == true
				|| lastName.isBlank() == true || email.isBlank() == true) {
			return statusCode;
		}

		OkHttpClient client = new OkHttpClient();

		RequestBody body = new MultipartBody.Builder().addFormDataPart("username", username)
				.addFormDataPart("password", password).addFormDataPart("firstname", firstName)
				.addFormDataPart("lastname", lastName).addFormDataPart("email", email).build();
	...
	/**
	 * Refreshes the access token for the user. Intended to be ran when an access
	 * token is expired, but will still refresh if the token is in-date. Will update
	 * the saved access
	 * 
	 * @param user The user whose access token is to be refreshed.
	 * @return Returns the status code (standard HTTP codes, e.g. 200 success, 403
	 *         incorrect credentials), or 0 if an error occurs.
	 * @throws AuthenticationException Occurs when the refresh token has expired -
	 *                                 try logging in.
	 * @throws IOException             Most likely to occur when no refresh token
	 *                                 exists, as the user is logging in for the
	 *                                 first time. Could occur due to network errors
	 *                                 - check server.
	 */
	public int refreshAccessToken(User user) throws IOException, AuthenticationException {
		...
		OkHttpClient client = new OkHttpClient();
		Request request = new Request.Builder().url(refreshURL).header("Authorization", "Bearer " + refreshToken)
				.build();
		...
		if (statusCode == 200) {
			...
			throw new AuthenticationException(
					"Error: server returned 403 forbidden. Refresh token possibly expired - try logging in.");
		} else if (statusCode == 500) {
			throw new RuntimeException("Unknown server error - check server logs");
		} else {
			return 0;
