...
public class HashtagService {
	...
	/**
	 * Retrieves hashtags from the server and returns them as an array of
	 * EventMarkers.
	 * 
	 ...
	 */
	public static ArrayList<EventMarker> retrieveHashtagsAsEventMarkers(MapScene mapScene, Bitmap bitmap)
		...
		if (statusCode == 200) {
			// Success! Now handle the hashtag conversion to EventMarkers.
	 ...
	 * @param authToken the authorisation token for the user. Must be admin or
	 *                  verified or 403 error will occur.
	 * @return ArrayList<EventMarker> ArrayList of the EventMarkers.
	 */
	public static EventMarker updateHashtagName(EventMarker hashtag, String newName, String authToken)
			throws IOException, AuthenticationException {

		OkHttpClient client = new OkHttpClient();

		RequestBody body = new MultipartBody.Builder().addFormDataPart("name", hashtag.getName())
				.addFormDataPart("newName", newName).build();

		Request request = new Request.Builder().url(hashtagURL).addHeader("Authorization", "Bearer " + authToken)
				.post(body).build();
